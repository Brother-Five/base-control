...
// ¡®order¡¯ [1..32] is the CRC polynom order, counted without the leading ¡¯1¡ä bit
// ¡®polynom¡¯ is the CRC polynom without leading ¡¯1¡ä bit
...
unsigned int CRC_Soft32Check (unsigned char* p, unsigned long len, uint32_t CRC32Rec) {
	...
	crc = (crc << 8) ^ crctab[ ((crc >> (order-8)) & 0xff) ^(unsigned char)((CRC32Rec>>24) &0xff)];
	crc = (crc << 8) ^ crctab[ ((crc >> (order-8)) & 0xff) ^(unsigned char)((CRC32Rec>>16) &0xff)];
	crc = (crc << 8) ^ crctab[ ((crc >> (order-8)) & 0xff) ^(unsigned char)((CRC32Rec>>8) &0xff)];
	crc = (crc << 8) ^ crctab[ ((crc >> (order-8)) & 0xff) ^(unsigned char)((CRC32Rec) &0xff)];
	crc^= 0x00000000;
	crc&= 0xFFFFFFFF;
	return(crc);
}
...
void UsasrFrameSend(FRAME * pBuffer)
{
	...
	pBuffer->CRC32 = CRC_Soft32(pBuffer->DATA,pBuffer->LEN + pBuffer->MOD);
...
unsigned char recdemo(unsigned char * str)
{
	...
	if(CRC_Soft32Check( ((FRAME *)str) ->DATA , ((FRAME *)str)->LEN + ((FRAME *)str)->MOD,((FRAME *)str)->CRC32)!= 0)
	{
		return 1;
	}
	printf("value = %08X\r\n",CRC_Soft32 (((FRAME *)str) ->DATA, ((FRAME *)str)->LEN + ((FRAME *)str)->MOD));
	return 0;
}
